...
{
	public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture) => $"   {((DateOnly)values[0] != DateOnly.MaxValue ? $"{((DateOnly)values[0]).ToRelativeDays() switch { < 0 => "expired", 1 => "1 day", 7 => "1 week", 14 => "2 weeks", 21 => "3 weeks", < 28 => $"{((DateOnly)values[0]).ToRelativeDays()} days", _ => ((DateOnly)values[0]).ToRelativeDays() / 28 is 1 ? "1 month" : $"{((DateOnly)values[0]).ToRelativeDays() / 28} months" }} - {(((DateOnly)values[0]).ToRelativeDays() / 28 is 0 ? $"{(DateOnly)values[0]:d MMMM}" : $"{(DateOnly)values[0]:MMMM y}")}".ToLower() : "no expiry date")} - {((bool)values[1] ? "opened" : "unopened")}";
